...
def confusion_matrix_and_report(X, y, pipeline, label_map):
    ...
    report = classification_report(y, prediction, target_names=label_map,
                                   output_dict=True)
...
def FullDiscretPipelineFinal():
    best_pipeline = Pipeline([
        ...
        ('encoder', dm.OrdinalEncoder(encoding_method='arbitrary',
...
def predict_step_count_task():
    ...
    if uploaded_file is not None:
        ...
        placeholder.write("### Please wait for the model to be"
                          " trained and predictions to be made...")
        ...
        label_names = ['<3524', '3525-7048', '7049-10572', '10573-14096',
                       '14097-17620', '17621-20000', '20000+']

        st.write(f"* The classes represent the following"
                 f" ranges: \n{label_names} \n")
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            df_discret.drop(columns=['Step Count']),
        ...
        label_map = ['<3524', '3525-7048', '7049-10572', '10573-14096',
                     '14097-17620', '17621-20000', '20000+']
        ...
        st.write(f"* The pipeline has"
                 f" {len(columns_after_data_cleaning_feat_eng)} features"
        ...
        best_features = [
            columns_after_data_cleaning_feat_eng[i] for i in range(
                len(support)) if support[i]]
        ...
        st.write(f"* These are the {len(best_features)}"
                 " most important features in descending order. "
                 "The model was trained on them:"
                 f" \n{df_feature_importance['Feature'].to_list()}")

        fig, ax = plt.subplots()
        df_feature_importance.plot(kind='bar', x='Feature',
                                   y='Importance', ax=ax)
