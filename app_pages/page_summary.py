...
def page_summary_body():
    ...
    st.write(
        f"* For additional information, please visit and **read** the README.md file in the project's GitHub repository."
        f"* https://github.com/MaxBWiseman/Predictive_Analytics_Project "
    )
    ...
    st.success(
        "**Business Requirements**\n"
        "A company wishes to:\n"
        "* Segment users into different groups based on health and activity data collected from smart watch sensors.\n"
        "* Target these segments with specific market advertisements for fitness products, health supplements, or health/wellness classes/programs.\n"
        "* Conduct research on the correlation between various health metrics (e.g., heart rate, activity level, stress level) to improve customer experience with targeted advertisements and product suggestions."
    )

    # Hypothesis 1
    st.success(
        "**Hypothesis 1**\n"
        "* Users can be segmented into distinct groups based on their health and activity data.\n"
    ...
    st.success(
        "**Hypothesis 3**\n"
        "* Users with high or low step counts are expected to find either positive or negative correlations with other smart-watch health variables. Try and predict step counts if applicable.\n"
        "* Significance Level/Alpha: 5%\n"
